define(['/hzml/common/js/config.js'], function () {
    require(['jquery', 'customtable', 'popups', 'pipups', 'common','nav'], function ($, CustomTable) {
        var form = layui.form;
        ...
        $("#exportReport").click(function () {
            ...
            if(data.length > 0){
                Pipups.confirm('确定要导出选中的记录？',function(){
                    var body = []
                    for (var i = 0; i < data.length; i++) {
                        body.push(data[i].id);
                    }
                    window.location.href = "/hzml/api/Export/exportReport?ids="+body;
                })
            }
        })

        //导出公示
        $("#exportPublic").click(function () {
            var data = CustomTable.getSelect();
            if (data.length == 0) {
                layer.msg("请选择最少一条数据", {time: 1000});
                return;
            ...
            if(data.length > 0){
                for(var i=0; i<data.length; i++){
                    //省级只能导出待省级公示、市级已公示
                    if(user.orgCode==1 && data[i].status!=3 && data[i].status!=2){
                        layer.msg("只能导出状态为“待省级公示“、“市级已公示”的报表！",{time: 2000});
                        return;
                    }
                    //市级只能导出待市级公示、市级已公示
                    if(user.orgCode==2 && data[i].status!=1 && data[i].status!=2){
                        layer.msg("只能导出状态为“待市级公示“、“市级已公示”的报表！",{time: 2000});
                        return;
                    }
                }
                Pipups.confirm('确定要导出选中的记录？',function(){
                    ...
                    window.location.href = "/hzml/api/Export/exportPublic?ids="+body;
                })
            }
        })

    ...
    require(['jquery', 'customtable', 'pipups', 'popups', 'common'], function ($, CustomTable) {
        ...
        $("#back").click(function(){
            ...
            Pipups.confirm('确定要退回流程吗?',function(){
                Helper.Ajax("Manage/provinceBack", body,function(result){
                    if(result.response.staticCode == 0){
        ...
        $("td").change(function(){
            ...
            if(p=='p1' || p=='p2' || p=='p3' || p=='p4' || p=='p8' || p=='p9'){
                ...
                //总分值大了，肯听填错数字了
                if(parseInt($(cur[3]).find("input[name='cityScore']").val()) > parseInt($(cur[2]).find("input[name='cityScore']").val())
                    || $(cur).find("input[name='cityScore']").val()<0 || $(cur[0]).find("input[name='cityScore']").val()=='NaN'){
            ...
            if(p.indexOf("p5")==0){
                ...
                $(cur[0]).find("input[name='cityScore']").val(parseInt(level1 * count));
                //总分值大了，肯听填错数字了
                if(parseInt($(cur[2]).find("input[name='cityScore']").val()) > parseInt($(cur[1]).find("input[name='cityScore']").val())
                    || $(cur).find("input[name='cityScore']").val()<0 || $(cur[0]).find("input[name='cityScore']").val()=='NaN'){
                    layer.msg("数据填写错误",{time:1000})
                    cur.find("input[name='cityScore']").val(0); //置零
                }
                ...
                $("#p5").find("input[name='cityScore']").val(parseInt(c1)+parseInt(c2)+parseInt(c3));
            }
